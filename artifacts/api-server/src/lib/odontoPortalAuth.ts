...
export type PortalPrincipal = { id: string; username: string; displayName: string; role: PortalRole; accountType: PortalAccountType; managerId: string | null; workspaceOwnerId: string };
...
export function publicUser(user: PortalPrincipal) {
  return { id: user.id, username: user.username, displayName: user.displayName, role: user.role, accountType: user.accountType, managerId: user.managerId, workspaceOwnerId: user.workspaceOwnerId };
}

export async function bootstrapAdmin() {
  if (bootstrapComplete) return;
  await db.execute(sql`ALTER TABLE odonto_portal_users ADD COLUMN IF NOT EXISTS account_type text NOT NULL DEFAULT 'individual'`);
  await db.execute(sql`ALTER TABLE odonto_portal_users ADD COLUMN IF NOT EXISTS manager_id text`);
  await db.execute(sql`ALTER TABLE odonto_portal_users ADD COLUMN IF NOT EXISTS workspace_owner_id text`);
  await db.execute(sql`UPDATE odonto_portal_users SET account_type = CASE WHEN role = 'admin' THEN 'creator' ELSE 'individual' END WHERE account_type IS NULL`);
  await db.execute(sql`UPDATE odonto_portal_users SET workspace_owner_id = id WHERE workspace_owner_id IS NULL`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS odonto_portal_users_manager_idx ON odonto_portal_users (manager_id)`);
  await db.execute(sql`CREATE INDEX IF NOT EXISTS odonto_portal_users_workspace_idx ON odonto_portal_users (workspace_owner_id)`);
  ...
  if (!password) return;
  const [existing] = await db.select({ id: odontoPortalUsers.id, accountType: odontoPortalUsers.accountType, passwordHash: odontoPortalUsers.passwordHash }).from(odontoPortalUsers).where(eq(odontoPortalUsers.username, username)).limit(1);
  if (existing) {
    const passwordChanged = !(await passwordMatches(password, existing.passwordHash));
    await db.update(odontoPortalUsers).set({ role: "admin", accountType: "creator", managerId: null, workspaceOwnerId: existing.id, ...(passwordChanged ? { passwordHash: await passwordHash(password) } : {}) }).where(eq(odontoPortalUsers.id, existing.id));
    if (passwordChanged) await db.delete(odontoPortalSessions).where(eq(odontoPortalSessions.userId, existing.id));
...
export async function attachPortalUser(req: PortalRequest, _res: Response, next: NextFunction) {
  ...
  try {
    const [session] = await db
      .select({ id: odontoPortalUsers.id, username: odontoPortalUsers.username, displayName: odontoPortalUsers.displayName, role: odontoPortalUsers.role, accountType: odontoPortalUsers.accountType, managerId: odontoPortalUsers.managerId, workspaceOwnerId: odontoPortalUsers.workspaceOwnerId })
      ...
      .limit(1);
    if (session) req.portalUser = { ...session, role: session.role === "admin" ? "admin" : "member", accountType: (["creator", "manager", "member", "individual"].includes(session.accountType) ? session.accountType : "individual") as PortalAccountType, workspaceOwnerId: session.workspaceOwnerId || session.id };
  } catch {
...
export function requirePortalManager(req: PortalRequest, res: Response): PortalPrincipal | null {
  const user = requirePortalUser(req, res);
  if (!user) return null;
  if (user.accountType === "creator" || user.accountType === "manager") return user;
  res.status(403).json({ error: "Você não tem permissão para gerenciar usuários." });
  return null;
}
