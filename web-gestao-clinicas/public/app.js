...
function dailyGuideMessages(cl,raw,people=[],history={},sourceDate=isoDay()){
 ...
 const conversionTask='Trabalhe a Ferramenta de Conversão dos últimos 45 dias e agende as oportunidades abertas. '+(evalsNeeded!=null?'Para sustentar '+chairDaily+' efetivações CG com 80% de aceitação, precisamos de cerca de '+evalsNeeded+' avaliações comparecidas por dia útil; confira a agenda de hoje e complete a diferença. ':'')+'Me retorne contatos, agendamentos e comparecimentos.';
 const indicationTask='Trabalhe de forma enérgica a coleta em todos os atendimentos: explique a importância da indicação e o benefício vigente de 7% de desconto nas parcelas pela campanha Amigo do Peito. '+(x.amigoPoss!=null?'Hoje temos '+x.amigoPoss+' possibilidades'+(x.amigoInd!=null?' e apenas '+x.amigoInd+' contatos/indicações registrados':'')+'. ':'')+'Quanto mais números coletarmos, mais possibilidades teremos na Ferramenta de Conversão. Quero o total de contatos novos e de avaliações agendadas.';
 const agendaTask='Confirme a agenda, recupere os faltosos'+(x.missedYesterday!=null?' ('+x.missedYesterday+' no último dia medido)':'')+' e priorize os tratamentos efetivados sem início. Me retorne os reagendamentos e os inícios.';
 const assignment=team.map((p,i)=>{const f=String(p.functions||'').toLowerCase();let task=/convers/i.test(f)?conversionTask:/orto|agenda/i.test(f)?agendaTask:i%2===0?conversionTask:indicationTask;const cg=chairDaily==null?null:Math.floor(chairDaily/team.length)+(i<chairDaily%team.length?1:0);const ortho=orthoDaily==null?null:Math.floor(orthoDaily/team.length)+(i<orthoDaily%team.length?1:0);return {name:p.name.trim(),task,cg,ortho}});
 ...
 const capture=x.amigoPct!=null?'A campanha Amigo do Peito está em '+Math.round(x.amigoPct)+'%'+(x.amigoPoss!=null?' ('+(x.amigoInd??'—')+' indicações em '+x.amigoPoss+' possibilidades)':'')+'. '+(newContacts>0?'Para o primeiro patamar de 10% das possibilidades, faltam '+newContacts+' indicações registradas. ':''):'';
 const praise=[x.rebooking!=null&&x.rebooking>=90?'Parabéns pelo reagendamento de '+Math.round(x.rebooking)+'% no último dia medido. ':'',x.app!=null&&x.app>=90?'O uso do aplicativo está em '+Math.round(x.app)+'%; mantenham a constância. ':''].join('');
 const healthyTicket=x.ticketHealthy||3488.25;
 const daily='Pelo Mapa até '+brDate(x.periodEnd)+', faltam '+(chairGap!=null?chairGap+' efetivações de CG':'efetivações de CG a conferir')+(financialGap!=null?' e '+centralMoney(financialGap)+' em CG':'')+(orthoGap!=null?' e '+orthoGap+' pastas pagas de Orto':'')+'. Restam '+days+' dias úteis; o ritmo da clínica é de '+(chairDaily!=null?chairDaily+' pacientes de CG':'CG a confirmar')+(orthoDaily!=null?' e '+orthoDaily+' pastas pagas de Orto':'')+' por dia útil. ';
 const agendaToday=x.todayEvaluations!=null?'Iniciamos o dia com '+x.todayEvaluations+' avaliações na agenda. '+(evalsNeeded!=null&&x.todayEvaluations<evalsNeeded?'Elas não são suficientes para o ritmo que precisamos hoje; faltam pelo menos '+Math.max(0,evalsNeeded-x.todayEvaluations)+' avaliações para sustentar a meta. ':''):'Confira agora quantas avaliações realmente temos na agenda de hoje; não vou presumir esse número sem fonte. ';
 const individual=assignment.map(a=>{const first=a.name.split(/\s+/)[0],greeting=first.slice(0,1).toLocaleUpperCase('pt-BR')+first.slice(1).toLocaleLowerCase('pt-BR');const target='Sua meta individual de hoje é '+(a.cg>0?a.cg+' '+(a.cg===1?'efetivação':'efetivações')+' de Clínico Geral, que '+(a.cg===1?'representa':'representam')+' '+centralMoney(a.cg*healthyTicket)+' ('+a.cg+' × ticket médio saudável de '+centralMoney(healthyTicket)+')':a.cg===0?'apoiar a agenda e os fechamentos de CG':'efetivações de Clínico Geral a definir')+(a.ortho>0?' e '+a.ortho+' '+(a.ortho===1?'pasta paga':'pastas pagas')+' de Orto':'')+'. ';return {recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+greeting+', tudo bem? '+yesterday+agendaToday+'A meta de hoje pede '+(chairDaily!=null?chairDaily+' pacientes efetivados de Clínico Geral':'a meta de CG a confirmar')+(chairDaily!=null?', totalizando aproximadamente '+centralMoney(chairDaily*healthyTicket):'')+(orthoDaily!=null?', e '+orthoDaily+' '+(orthoDaily===1?'pasta':'pastas')+' de Ortodontia':'')+'. '+(chairDaily!=null&&chairDaily>=8?'É um número alto, então precisamos atacar coleta e conversão com energia. ':'Vamos concentrar o trabalho em coleta, conversão e agenda. ')+capture+target+a.task+' '+praise+'Vamos manter constância nessa tarefa hoje, combinado? Você sabe qual é o percentual saudável da aceitação? Na parcial, me traga contatos coletados, avaliações agendadas, comparecimentos e efetivações.'}});
