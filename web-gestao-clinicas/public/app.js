...
function dailyGuideMessages(cl,raw,people=[],history={},sourceDate=isoDay()){
 ...
 const individual=assignment.map(a=>{
   ...
   if(role===0){
     ...
     return {recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+g+', tudo bem? '+simpleYesterday+current+need+accept+'Hoje preciso da sua ajuda principalmente com os agendamentos de avaliação: trabalhe a conversão e deixe '+evalTargetText+' novas avaliações agendadas entre hoje e amanhã, sem passar de 15. O mais importante é não encerrar o dia com horários de avaliação vazios. '+clinicPraise+'No meio do dia me diga quantas avaliações você já conseguiu agendar, combinado?'};
   }
   if(role===1){
     ...
     return {recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+g+'! Tudo bem? '+absent+noAgenda+rebook+'Hoje quero você cuidando dos faltosos, olhando a agenda dos profissionais e chamando quem ainda está sem o primeiro/próximo agendamento. A ideia é aproveitar os horários disponíveis e não deixar paciente parado. '+clinicPraise+'Na parcial me passe quantos pacientes você recuperou e quantos ficaram agendados, combinado?'};
   }
 ...
 const group='Bom dia Drs. como vcs estão ??? Hoje alinhei três pontos com as meninas para atacarmos durante o dia: 1) deixar as avaliações de hoje e amanhã preenchidas, com meta de até 15 novos agendamentos; 2) trabalhar os faltosos, conferir a agenda dos profissionais e chamar os pacientes que estão sem primeiro ou próximo agendamento; 3) revisar a cobrança das pastas de Ortodontia atrasadas dos últimos 3 meses, acompanhando tentativa, acordo e pagamento. A ideia é cada uma cuidar bem da sua frente, sem sobrecarregar ninguém, e no fim do dia vermos o que avançou. Se precisarem de apoio em algum desses pontos, me chamem, combinado?' ;
