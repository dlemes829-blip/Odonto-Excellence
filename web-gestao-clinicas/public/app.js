...
function dailyGuideMessages(cl,raw,people=[]){
 ...
 const treatment='Drs., olhando os 21 pontos do relatório, eu priorizaria o que mexe direto na cadeira e no caixa. '+(gap>0?'Cadeira: '+(x.chairEff??'—')+'/'+(x.chairProp??'—')+', gap de '+gap+'. ':'Cadeira proporcional entregue. ')+(x.ticket!=null?'Ticket: '+centralMoney(x.ticket)+' vs saudável '+centralMoney(ticketRef)+'. ':'')+'Orto: '+(x.orthoEff??'—')+'/25 efetivações e '+(x.orthoPaid??'—')+'/18 pastas pagas. Aceitação: '+(x.accept??'—')+'%. Plano de ação: 1) recepção transforma o gap em lista nominal de avaliações, faltosos e reativações; 2) cada avaliação sem fechamento recebe follow-up com motivo e próximo contato; 3) profissionais trabalham plano completo pelas 4 odontologias para proteger ticket, sem empurrar tratamento; 4) Orto recebe meta diária de avaliações, fechamento e pasta paga; 5) parcial à tarde com realizado x meta e redistribuição do que não andou. Ponto negativo sem responsável e próxima ação não entra como tratativa.';
 return {individual,group,ret,treatment};
}
async function saveDailyGuides(id){
 ...
 const g=dailyGuideMessages(cl,raw,people),rows=[...g.individual.map(x=>({section:'morning_messenger',...x})),{section:'morning_group',recipient:'group',title:'Grupo · manhã',message:g.group},{section:'afternoon_return',recipient:'group',title:'Retorno da tarde',message:g.ret},{section:'treatment',recipient:'group',title:'Tratativa estratégica',message:g.treatment}];
...
async function bindGuideCards(){$('[data-copy-guide]').forEach(b=>b.onclick=()=>copyText(b.closest('.guide-output').querySelector('.message-box').textContent));$('[data-guide-image]').forEach(inp=>inp.onchange=async()=>{const f=inp.files?.[0];if(!f)return;const d=await fileToDataUrl(f);await mgmtFetch('daily_clinic_guides?id=eq.'+inp.dataset.guideImage,{method:'PATCH',body:JSON.stringify({image_data:d,updated_at:new Date().toISOString()})});toast('Imagem anexada à mensagem.');renderPage()})}
function renderPeriod1(c){
 const id=state.selectedClinic||1,cl=clinic(id),m=metric(id),p=plan(id)||{collaborator_count:2},pri=priorities(m),staff=data.staff[id]||[],n=Math.min(5,Math.max(2,p.collaborator_count||2));
 c.innerHTML=`<div class="period-head"><div><span class="period-pill active">MANHÃ</span><h3>Mensageiro + grupo + franqueado</h3><p>Comece o dia com meta, direção e solução. A tarde será usada para cobrar o retorno do que foi combinado.</p></div><select id="p1Clinic">${CLINICS.map(x=>`<option value="${x.id}" ${x.id===id?'selected':''}>${esc(x.name)}</option>`).join('')}</select></div>
 <div class="guide-strip"><div><strong>1 · Mensageiro</strong><span>Conversa personalizada · dor · treinamento · confirmação</span></div><div><strong>2 · Grupo</strong><span>Problema + solução + direcionamento para equipe/franqueado</span></div><div><strong>3 · Franqueado</strong><span>Resumo breve das metas e do que precisa acompanhar</span></div></div>
 <div class="split"><div class="card"><div class="section-head"><div><span class="eyebrow">GRUPO</span><h3>Comunicação estratégica</h3></div><button id="copyMorningGroup" class="ghost">Copiar</button></div><div id="morningGroup" class="message-box">${esc(morningGroupText(cl,m,pri))}</div><div class="guide-note"><strong>Regra:</strong> nunca levar só o problema. Dizer o que foi direcionado, quem precisa agir e qual solução será acompanhada.</div></div>
 <div class="card"><div class="section-head"><div><span class="eyebrow">FRANQUEADO</span><h3>Metas da manhã</h3></div><button id="copyP1Franchise" class="ghost">Copiar</button></div><div id="p1Franchise" class="message-box">${esc(franchiseP1(cl,m,pri))}</div><div class="guide-note">Mensagem breve para o franqueado saber o foco e reforçar a execução com a equipe.</div></div></div>
 <div class="card"><div class="section-head"><div><span class="eyebrow">MENSAGEIRO</span><h3>Conversas individuais</h3><p>Personalize por colaboradora: explicar a dor, orientar como resolver e pedir retorno do que foi feito.</p></div><button id="saveP1" class="btn primary">Salvar rotina da manhã</button></div><div id="p1Staff"></div></div>
 <div class="card guide-card"><div class="section-head"><div><span class="eyebrow">GUIA DA SUPERVISÃO</span><h3>Critério para as tratativas</h3></div><button id="openTreatment" class="btn secondary">Ir para Tratativa</button></div><div class="guide-rules"><div><strong>Aceitação</strong><span>Mensageiro: tratar abordagem, nomes e retorno. Grupo: menor %, recepção, dentistas e reforço do franqueado.</span></div><div><strong>Ticket médio</strong><span>Tratar no grupo com o franqueado e profissionais. Referência saudável: ${money(data.settings.ticketReference)}. Trabalhar valor tabela, média de procedimentos e as 4 odontologias: necessidade, reparadora, prevenção e desejo.</span></div><div><strong>Retorno da tarde</strong><span>Comparar meta da manhã com o realizado. Cobrar o que falta e definir a ação para fechar o dia.</span></div></div></div>`;
 $('#p1Clinic').onchange=e=>{state.selectedClinic=Number(e.target.value);renderPeriod1(c)};
 $('#copyMorningGroup').onclick=()=>copyText($('#morningGroup').textContent);
 $('#copyP1Franchise').onclick=()=>copyText($('#p1Franchise').textContent);
 $('#openTreatment').onclick=()=>go('centralReturns');
 const wrap=$('#p1Staff');
 for(let i=0;i<n;i++){const st=staff[i]||{},resp=st.resp||defaultResponsibility(i,pri),target=st.target||defaultTarget(resp);const row=document.createElement('div');row.className='assignment p1-row';row.innerHTML=`<div><label>Nome</label><input class="p1-name" value="${esc(st.name||'')}" placeholder="Colaboradora ${i+1}"></div><div><label>Foco</label><select class="p1-resp">${['Avaliações','Efetivações','Conversão','Faltosos','Trat. sem 1º agendamento','App','Pesquisa satisfação','Ortodontia'].map(x=>`<option ${x===resp?'selected':''}>${x}</option>`).join('')}</select></div><div><label>Meta</label><input class="p1-target" value="${esc(target)}"></div><div class="p1-message"><label>Mensagem individual</label><div class="message-box"></div></div><button class="ghost p1-copy">Copiar</button>`;wrap.appendChild(row);const update=()=>{const name=$('.p1-name',row).value,r=$('.p1-resp',row).value,t=$('.p1-target',row).value;$('.message-box',row).textContent=secretaryText(cl,id,i,name,r,t)};$$('input,select',row).forEach(x=>x.oninput=update);$('.p1-copy',row).onclick=()=>copyText($('.message-box',row).textContent);update()}
 $('#saveP1').onclick=()=>savePeriod1(id);
}
...
function renderReturns(c){
 c.innerHTML=`<div class="page-intro"><div><span class="eyebrow">ACOMPANHAMENTO</span><h3>Retorno da tarde</h3><p>O texto salvo da clínica também vira guia diário para comparar o combinado com o realizado.</p></div></div>
 <div class="card"><div class="form-grid"><div class="span-2"><label>Clínica</label><select id="r_clinic">${CLINICS.map(x=>`<option value="${x.id}" ${x.id===state.selectedClinic?'selected':''}>${esc(x.name)}</option>`).join('')}</select></div><div class="span-2"><label>Texto completo do relatório</label><textarea id="r_report_text" rows="9" placeholder="Cole o relatório em texto desta clínica…"></textarea></div></div><div class="actions"><button id="r_save_text" class="btn primary">Salvar texto desta clínica</button><span id="r_text_state" class="hint">Carregando…</span></div></div>
 <div class="split"><div class="card"><div class="form-grid"><div><label>Colaboradora</label><input id="r_collab"></div><div><label>Cumpriu?</label><select id="r_done"><option>Sim</option><option>Parcial</option><option>Não</option></select></div><div><label>Avaliações novas</label><input id="r_eval" type="number" value="0"></div><div><label>Efetivações novas</label><input id="r_eff" type="number" value="0"></div><div><label>Pastas Orto</label><input id="r_ortho" type="number" value="0"></div><div><label>Agendamentos</label><input id="r_apps" type="number" value="0"></div><div class="span-2"><label>O que falta / ação imediata</label><textarea id="r_pending" placeholder="Ex.: faltam 2 avaliações; trabalhar reativação e confirmar agenda das 18h"></textarea></div></div><button id="saveReturn" class="btn primary block">Salvar retorno da tarde</button></div><div class="card"><h3>Guia estratégico do dia</h3><div id="rGuide" class="message-box"></div><h3 style="margin-top:18px">Resumo do retorno</h3><div id="rSummary" class="message-box"></div></div></div>`;
 const loadText=async()=>{state.selectedClinic=Number($('#r_clinic').value);const rows=await mgmtFetch('central_report_texts?select=id,report_text,updated_at&capture_date=eq.'+isoDay()+'&clinic_id=eq.'+state.selectedClinic+'&return_type=eq.2&limit=1');$('#r_report_text').value=rows[0]?.report_text||'';$('#r_report_text').dataset.rowId=rows[0]?.id||'';$('#r_text_state').textContent=rows[0]?'Salvo hoje':'Ainda não salvo';gen()};
 const gen=()=>{const cl=clinic(Number($('#r_clinic').value));$('#rGuide').textContent=returnGuideText(cl,$('#r_report_text').value);$('#rSummary').textContent=`${cl.city}: ${$('#r_eval').value} avaliações novas, ${$('#r_eff').value} efetivações, ${$('#r_ortho').value} pastas Orto e ${$('#r_apps').value} agendamentos. Pendência: ${$('#r_pending').value||'nenhuma informada'}.`};
 $('#r_clinic').onchange=loadText;$$('#content input,#content select,#content textarea').forEach(x=>x.oninput=gen);
 $('#r_save_text').onclick=async()=>{const text=$('#r_report_text').value.trim();if(!text)return toast('Cole o texto primeiro.');const id=$('#r_report_text').dataset.rowId,body={capture_date:isoDay(),clinic_id:Number($('#r_clinic').value),return_type:2,report_text:text,updated_at:new Date().toISOString()};if(id)await mgmtFetch('central_report_texts?id=eq.'+id,{method:'PATCH',body:JSON.stringify(body)});else await mgmtFetch('central_report_texts',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(body)});toast('Texto salvo. Tratativa e retorno usarão a mesma base.');await loadText()};
 $('#saveReturn').onclick=()=>{gen();data.returns.unshift({id:`r-${Date.now()}`,day:isoDay(),month:monthKey(),clinic_id:Number($('#r_clinic').value),collaborator:$('#r_collab').value,completed_status:$('#r_done').value,new_evaluations:Number($('#r_eval').value||0),new_effectivations:Number($('#r_eff').value||0),ortho_folders:Number($('#r_ortho').value||0),appointments:Number($('#r_apps').value||0),pending:$('#r_pending').value,summary:$('#rSummary').textContent,guide:$('#rGuide').textContent});save();toast('Retorno da tarde registrado')};loadText();
}
