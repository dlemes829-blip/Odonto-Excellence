...
function dailyGuideMessages(cl,raw,people=[],history={},sourceDate=isoDay()){
 ...
 const assignment=team.map((p,i)=>{const f=String(p.functions||'').toLowerCase();let task=/avalia|convers/i.test(f)?conversionTask:/orto|cl[ií]nico|agenda/i.test(f)?agendaTask:i%2===0?conversionTask:indicationTask;return {name:p.name.trim(),task}});
 if(assignment.length===1)assignment[0].task=conversionTask+' '+indicationTask;
 const capture=x.capturePct!=null?'A coleta de indicações aparece em '+Math.round(x.capturePct)+'% ('+x.indications+'/'+x.cadastros+' cadastros no Mapa); precisamos entender onde o pedido está falhando. ':'';
 ...
 const daily='Pelo Mapa até '+brDate(x.periodEnd)+', faltam '+(chairGap!=null?chairGap+' efetivações de CG':'efetivações de CG a conferir')+(financialGap!=null?' e '+centralMoney(financialGap)+' em CG':'')+(orthoGap!=null?' e '+orthoGap+' pastas pagas de Orto':'')+'. Para fechar setembro nos '+days+' dias úteis restantes, o ritmo calculado é de '+(chairDaily!=null?chairDaily+' pacientes de CG':'CG a confirmar')+(financialDaily!=null?' ('+centralMoney(financialDaily)+'/dia útil)':'')+(orthoDaily!=null?' e '+orthoDaily+' pastas pagas de Orto/dia útil':'')+'. ';
 const individual=assignment.map(a=>({recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+a.name+', tudo bem? '+yesterday+'No último Mapa, foram '+(x.evals??'—')+' avaliações e '+(x.eff??'—')+' efetivações de CG; a aceitação está em '+(x.accept!=null?Math.round(x.accept)+'%':'apuração pendente')+' frente à referência de 80%. '+daily+capture+a.task+' '+praise+'Me diga o que está travando e traga os números na parcial, combinado?'}));
 for(const p of collectors)individual.push({recipient:p.name.trim(),title:'Mensageiro · cobrança · '+p.name.trim(),message:'Bom dia, '+p.name.trim()+', tudo bem? No Mapa, a cobrança do último dia medido registra '+(x.charges??'—')+' cobranças e '+(x.charged??'—')+' registros de cobrados; aproveitamento do agente '+(x.collectionAgent!=null?Math.round(x.collectionAgent)+'%':'não informado')+' (mínimo de 65%). Confira quais contatos tiveram retorno, priorize negociações viáveis e me traga quantidade de acordos e pagamentos confirmados, separados de simples contatos. Onde está a maior objeção?'});
