...
function treatmentItems(cl,x,days,healthyTicket){
 const items=[],where='No Mapa de Trabalho até '+brDate(x.periodEnd)+', ',add=(score,title,fact,action,question)=>{
  items.push({score,title,message:'Bom dia, franqueados de '+cl.city+'. '+where+fact+' Confesso que esse ponto me preocupa. '+action+' '+question+' Vamos acompanhar na parcial e corrigir juntos, combinado?'});
 };
 if(x.accept!=null&&x.accept<80)add(100+(80-x.accept),'Aceitação e conversão','a aceitação de Clínico Geral está em '+Math.round(x.accept)+'% ('+(x.eff??'—')+'/'+(x.evals??'—')+' avaliações), abaixo dos 80% saudáveis.','Revisem com a equipe as avaliações não fechadas, objeções e propostas; organizem retorno às oportunidades dos últimos 45 dias.','Quantos contatos, avaliações e fechamentos cada responsável fará hoje?');
 ...
 if(gap>0)add(95+Math.min(25,gap/5),'Cadeiras e valor de CG','foram '+x.chairEff+' efetivações de '+x.chairMonthly+' pacientes previstos por cadeira; faltam '+gap+' em '+days+' dias úteis.','A clínica precisa de '+daily+' efetivações CG/dia útil, equivalentes a '+centralMoney(daily*healthyTicket)+' usando '+centralMoney(healthyTicket)+' de ticket saudável por paciente. Confiram agenda e capacidade real antes de distribuir a meta.','Qual é a agenda confirmada e quantas avaliações adicionais precisamos agendar hoje?');
 const og=x.orthoPaidGoal!=null&&x.orthoPaid!=null?Math.max(0,x.orthoPaidGoal-x.orthoPaid):null;
 if(og>0)add(92+Math.min(20,og),'Pastas pagas de Orto','temos '+x.orthoPaid+' pastas pagas de '+x.orthoPaidGoal+' previstas; faltam '+og+'.','Precisamos do ritmo de '+Math.ceil(og/days)+' pastas pagas por dia útil. Revisem as avaliações pendentes e o caminho da proposta à pasta efetivamente paga.','Quem acompanhará cada pendência até o pagamento?');
 if(x.amigoPct!=null&&x.amigoPct<50){const target=x.amigoPoss!=null&&x.amigoInd!=null?Math.max(0,Math.ceil(x.amigoPoss*.1)-x.amigoInd):null;add(88+(50-x.amigoPct)/2,'Coleta e indicações','a campanha Amigo do Peito registra '+Math.round(x.amigoPct)+'% diante da referência de 50%'+(x.amigoPoss!=null?' e '+x.amigoInd+' indicações em '+x.amigoPoss+' possibilidades':'')+'.','Reforcem o pedido de indicação em cada atendimento e registrem contatos com autorização do paciente.'+(target!=null?' Para alcançar primeiro 10% das possibilidades atuais, faltam pelo menos '+target+' indicações registradas.':''),'Quantos contatos cada pessoa coletará e quantas avaliações vieram deles?')}
 if(x.treatmentsEff!=null&&x.treatmentsStarted!=null&&x.treatmentsStarted<x.treatmentsEff)add(85,'Tratamentos sem início','há '+x.treatmentsEff+' tratamentos efetivados e somente '+x.treatmentsStarted+' iniciados neste recorte.','Puxem a lista sem primeiro horário, ofereçam datas próximas e acompanhem confirmação e presença.','Quantos inícios conseguiremos marcar e realizar ainda hoje?');
 if(x.rebooking!=null&&x.rebooking<90)add(80+(90-x.rebooking)/2,'Reagendamento','o reagendamento de CG está em '+Math.round(x.rebooking)+'%, abaixo do mínimo de 90%.','Trabalhem os atendidos sem retorno marcado, as faltas e os cancelamentos antes de encerrar o dia.','Quantos pacientes estão pendentes e quem fará cada contato?');
 if(x.noAgendaCG!=null&&x.noAgendaCG>15)add(78+x.noAgendaCG/4,'Pacientes sem agenda','a proporção sem próximo agendamento em CG está em '+Math.round(x.noAgendaCG)+'%.','Priorizem pacientes em andamento sem retorno; preencham horários e confirmem com antecedência.','Qual o volume recuperado na próxima parcial?');
 if(x.collectionExecution!=null&&x.collectionExecution<80)add(72+(80-x.collectionExecution)/3,'Execução da cobrança','a execução de cobrança está em '+Math.round(x.collectionExecution)+'% diante dos 80% mínimos.','Separem tentativa, contato, negociação e pagamento confirmado; priorizem a carteira viável.','Quantos pagamentos e acordos efetivos vieram dos contatos?');
 if(x.app!=null&&x.app<90)add(68+(90-x.app)/4,'Aplicativo','o indicador de efetivações/instalações do aplicativo está em '+Math.round(x.app)+'%.','Oriente a instalação e confira com o paciente antes de sua saída, registrando o resultado real.','Quantas instalações concluídas tivemos na parcial?');
 if(x.satisfaction!=null&&x.satisfaction<90)add(65+(90-x.satisfaction)/5,'Pesquisa de satisfação','a pesquisa de satisfação registra '+Math.round(x.satisfaction)+'% frente à referência de 90%.','Peçam a resposta após o atendimento e registrem a abordagem sem presumir que ela foi concluída.','Quantas pesquisas foram solicitadas e respondidas?');
 if(x.cancelNotStarted!=null&&x.cancelNotStarted>18)add(75+(x.cancelNotStarted-18)/2,'Cancelamentos e inícios','o percentual de cancelamento não iniciado está em '+Math.round(x.cancelNotStarted)+'%, acima do máximo de 18%.','Revisem motivos, negociações e primeiro agendamento dos tratamentos recém-efetivados.','Que casos podemos recuperar hoje e qual apoio é necessário?');
 return items.sort((a,b)=>b.score-a.score).slice(0,8).map((v,i)=>({recipient:'item_'+String(i+1).padStart(2,'0'),title:(i+1)+'. '+v.title,message:v.message}));
}
...
function dailyGuideMessages(cl,raw,people=[],history={},sourceDate=isoDay()){
 ...
 const reception=people.filter(p=>p.role_type==='receptionist'&&p.name);
 const collectors=people.filter(p=>p.role_type==='collection_agent'&&p.name);
 const team=reception.length?reception:[{name:'Recepção',functions:''}];
 ...
 const yesterday=deltaCG!=null?'Finalizamos o dia de ontem com '+deltaCG+' '+(deltaCG===1?'efetivação':'efetivações')+' em Clínico Geral'+(deltaOrtho!=null?' e '+deltaOrtho+' '+(deltaOrtho===1?'efetivação':'efetivações')+' de Ortodontia':'')+'. '+((deltaCG===0||(deltaOrtho!=null&&deltaOrtho===0))?'O que houve? ':'Quero entender o que funcionou e o que ainda travou os demais fechamentos. '):'Antes do envio, confirme o fechamento de ontem em Clínico Geral e Ortodontia para não cobrarmos com dado presumido. ';
 ...
 const evalsNeeded=chairDaily!=null?Math.ceil(chairDaily/.8):null;
 const conversionTask='Trabalhe a Ferramenta de Conversão dos últimos 45 dias e transforme oportunidades em avaliações. '+(x.appNoSuccess!=null?'Temos '+x.appNoSuccess+' contatos sem sucesso no app; faça uma nova tentativa estratégica nos melhores horários. ':'')+'Me retorne tentativas, contatos efetivos e avaliações agendadas.';
 const indicationTask='Trabalhe a coleta em todos os atendimentos: explique a importância da indicação e o benefício vigente de 7% de desconto nas parcelas pela campanha Amigo do Peito. '+(x.amigoPoss!=null?'Temos '+x.amigoPoss+' possibilidades'+(x.amigoInd!=null?' e '+x.amigoInd+' indicações registradas':'')+'. ':'')+'Quero contatos novos e avaliações originadas dessa frente.';
 const agendaTask='Ataque agenda e continuidade: '+(x.noAgendaCG!=null?Math.round(x.noAgendaCG)+'% dos pacientes de CG estão sem próximo agendamento. ':'')+(x.missedYesterday!=null?'Ontem tivemos '+x.missedYesterday+' faltosos. ':'')+'Confirme agenda, recupere faltosos e deixe o próximo passo agendado. Me retorne reagendamentos e comparecimentos.';
 const startTask='Trabalhe os tratamentos efetivados que ainda não iniciaram. '+(x.treatmentsEff!=null&&x.treatmentsStarted!=null?'O Mapa mostra '+x.treatmentsEff+' efetivados e apenas '+x.treatmentsStarted+' iniciados. ':'')+'Identifique os casos viáveis, faça contato e me retorne quantos inícios foram recuperados.';
 const satisfactionTask='Hoje sua frente é experiência do paciente. '+(x.satisfaction!=null?'A pesquisa de satisfação está em '+Math.round(x.satisfaction)+'% diante dos 90% saudáveis. ':'')+'Garanta a abordagem dos pacientes atendidos e me retorne quantas pesquisas foram solicitadas e respondidas.';
 const appTask='Sua frente hoje é aplicativo e indicações digitais. '+(x.app!=null?'O indicador do app está em '+Math.round(x.app)+'%. ':'')+(x.appNoSuccess!=null?'Há '+x.appNoSuccess+' contatos sem sucesso nas indicações pelo aplicativo. ':'')+'Trabalhe instalação/uso e recupere contatos pendentes, retornando os números na parcial.';
 const frontPool=[conversionTask,indicationTask,agendaTask,startTask,satisfactionTask,appTask];
 const mapNames=new Set((x.collaborators||[]).map(v=>v.name));
 const assignment=team.map((p,i)=>{const f=String(p.functions||'').toLowerCase(),nm=p.name.trim().toUpperCase(),hasMap=mapNames.has(nm);let task;
   if(/convers/i.test(f))task=conversionTask;else if(/orto|agenda/i.test(f))task=agendaTask;else if(/app/i.test(f))task=appTask;else if(/satis/i.test(f))task=satisfactionTask;else task=frontPool[i%frontPool.length];
   const cg=chairDaily==null?null:Math.floor(chairDaily/team.length)+(i<chairDaily%team.length?1:0),ortho=orthoDaily==null?null:Math.floor(orthoDaily/team.length)+(i<orthoDaily%team.length?1:0);
   return {name:p.name.trim(),task,cg,ortho,index:i,hasMap};
 });
 if(assignment.length===1)assignment[0].task=conversionTask+' '+indicationTask;
 const newContacts=x.amigoPoss!=null&&x.amigoInd!=null?Math.max(0,Math.ceil(x.amigoPoss*.1)-x.amigoInd):null;
 const capture=x.amigoPct!=null?'A campanha Amigo do Peito está em '+Math.round(x.amigoPct)+'%'+(x.amigoPoss!=null?' ('+(x.amigoInd??'—')+' indicações em '+x.amigoPoss+' possibilidades)':'')+'. '+(newContacts>0?'Para chegar ao primeiro patamar de 10%, faltam '+newContacts+' indicações registradas. ':''):'';
 const clinicWins=[];if(x.accept!=null&&x.accept>=80)clinicWins.push('aceitação de CG em '+Math.round(x.accept)+'%');if(x.orthoAccept!=null&&x.orthoAccept>=80)clinicWins.push('aceitação de Orto em '+Math.round(x.orthoAccept)+'%');if(x.revenueDeltaPct!=null&&x.revenueDeltaPct>0)clinicWins.push('receita acima da referência anterior');if(x.indications!=null&&x.cadastros&&x.indications/x.cadastros>=.5)clinicWins.push('indicações em '+Math.round(x.indications/x.cadastros*100)+'% dos cadastros');
 const clinicPraise=clinicWins.length?'Parabéns por '+clinicWins.slice(0,2).join(' e ')+'. ':'';
 const healthyTicket=x.ticketHealthy||3488.25;
 const agendaToday=x.todayEvaluations!=null?'Iniciamos o dia com '+x.todayEvaluations+' avaliações na agenda. ':'Confira a agenda real de avaliações de hoje antes do envio; esse número não aparece de forma segura no Mapa. ';
 const openings=[
  (a,g)=>'Bom dia, '+g+', tudo bem? '+yesterday,
  (a,g)=>'Bom dia, '+g+'! Tudo certo? Quero alinhar sua frente de hoje. '+yesterday,
  (a,g)=>'Bom dia, '+g+', como você está? Olhando os números da clínica, '+yesterday.toLowerCase(),
  (a,g)=>'Bom dia, '+g+'! Vamos organizar seu foco de hoje. '+yesterday
 ];
 const individual=assignment.map(a=>{const first=a.name.split(/\s+/)[0],greeting=first.slice(0,1).toLocaleUpperCase('pt-BR')+first.slice(1).toLocaleLowerCase('pt-BR'),target=a.cg>0?'Sua contribuição para a meta de hoje é '+a.cg+' '+(a.cg===1?'efetivação':'efetivações')+' de CG'+(a.ortho>0?' e '+a.ortho+' '+(a.ortho===1?'pasta paga':'pastas pagas')+' de Orto':'')+'. ':a.ortho>0?'Sua contribuição de hoje é '+a.ortho+' '+(a.ortho===1?'pasta paga':'pastas pagas')+' de Orto. ':'Sua contribuição será medida pela frente operacional abaixo. ';
  let context=a.index===0?agendaToday+'A clínica precisa de '+(chairDaily??'—')+' efetivações de CG hoje'+(chairDaily!=null?', cerca de '+centralMoney(chairDaily*healthyTicket):'')+'. ':a.index===1?capture:a.index===2?((x.rebooking!=null?'O reagendamento fechou em '+Math.round(x.rebooking)+'%. ':'')+(x.absenteesPct!=null?'Os faltosos estão em '+Math.round(x.absenteesPct)+'%. ':'')):((x.startedPct!=null?'A relação de efetivados x iniciados está em '+Math.round(x.startedPct)+'%. ':''));
  let praise=a.index===0&&x.accept>=80?'Parabéns pela aceitação de '+Math.round(x.accept)+'%; precisamos preservar essa qualidade. ':a.index===1&&x.indications!=null&&x.cadastros&&x.indications/x.cadastros>=.5?'Ponto positivo: as indicações totais já representam '+Math.round(x.indications/x.cadastros*100)+'% dos cadastros. ':a.index===2&&x.rebooking>=90?'Parabéns pelo reagendamento em '+Math.round(x.rebooking)+'%. ':clinicPraise;
  const close=a.index%3===0?'Na parcial, me traga seus números e o principal obstáculo, combinado? Você sabe qual é o percentual saudável da aceitação?':a.index%3===1?'Quero sua parcial com resultado dessa frente e o que ainda está travando. Combinado?': 'Me atualize no meio do período com realizado x meta e o próximo passo, combinado?';
  return {recipient:a.name,title:'Mensageiro · '+a.name,message:openings[a.index%openings.length](a,greeting)+context+target+a.task+' '+praise+close};
 });
 for(const p of collectors){const name=p.name.trim(),dual=individual.find(v=>v.recipient.toLocaleLowerCase('pt-BR')===name.toLocaleLowerCase('pt-BR'));if(dual){dual.message+=' Na sua frente de cobrança, separe tentativas, acordos e pagamentos confirmados; me traga esses números na parcial.';continue}individual.push({recipient:name,title:'Mensageiro · cobrança · '+name,message:'Bom dia, '+name.split(/\s+/)[0]+', tudo bem? No Mapa, a cobrança do último dia medido registra '+(x.charges??'—')+' cobranças e '+(x.charged??'—')+' registros de cobrados; aproveitamento do agente '+(x.collectionAgent!=null?Math.round(x.collectionAgent)+'%':'não informado')+' (mínimo de 65%). Confira quais contatos tiveram retorno, priorize negociações viáveis e me traga quantidade de acordos e pagamentos confirmados, separados de simples contatos. Onde está a maior objeção?'})}
 const group='Bom dia, equipe e franqueados de '+cl.city+'. Leitura do Mapa até '+brDate(x.periodEnd)+'. '+yesterday+chairLine+moneyLine+orthoLine+focus+'Hoje vamos atuar na agenda, conversão dos últimos 45 dias, indicações e tratamentos sem início. '+clinicPraise+'Franqueados, onde precisamos do apoio de vocês? Na parcial, me tragam avaliações marcadas e comparecidas, CG e Orto efetivados, pastas pagas e inícios. O print original do Mapa segue anexado na tratativa.';
 const retIndividuals=individual.map(a=>({recipient:a.recipient,title:'Retorno · '+a.recipient,message:'Boa tarde, '+a.recipient+'. Sobre a frente combinada hoje, quanto foi realizado, o que ficou pendente e por quê? Me retorne os números e o próximo horário de ação; se houver obstáculo para a equipe, vamos envolver o franqueado.'}));
 ...
 const treatment='Diagnóstico · '+cl.city+' ('+source+'). Auditoria '+x.audit.count+'/21 itens; item 15 ausente na exportação. '+evidence+collectionText+focus+'Equipes: '+assignment.map(a=>a.name+' — '+a.task).join(' ')+' Comparação de períodos: '+(lastDay?reportHistorySummary(x,prev,null):'sem leitura anterior comparável para o período')+'. Critério: cadeira é contagem de efetivações; meta financeira de CG é em reais, calculada separadamente. O ROAD possui apuração própria; não igualar seus totais ao Mapa. Validar capacidade e números de hoje antes do envio.';
 return {individual,retIndividuals,group,ret,treatment,treatmentItems:treatmentItems(cl,x,days,healthyTicket),audit:x.audit,metrics:x};
}
...
async function saveDailyGuides(id){
 ...
 for(const r of rows){const body={guide_date:isoDay(),clinic_id:id,section:r.section,recipient:r.recipient,title:r.title,message:r.message,action_plan:r.section==='treatment_items'?r.message:''};const old=await mgmtFetch('daily_clinic_guides?select=id&guide_date=eq.'+isoDay()+'&clinic_id=eq.'+id+'&section=eq.'+encodeURIComponent(r.section)+'&recipient=eq.'+encodeURIComponent(r.recipient)+'&order=id.desc&limit=20');if(old[0]){await mgmtFetch('daily_clinic_guides?id=eq.'+old[0].id,{method:'PATCH',body:JSON.stringify(body)});for(const stale of old.slice(1))await mgmtFetch('daily_clinic_guides?id=eq.'+stale.id,{method:'DELETE'})}else await mgmtFetch('daily_clinic_guides',{method:'POST',body:JSON.stringify(body)})}
 const valid=new Set(rows.map(r=>r.section+'|'+r.recipient)),sections=['morning_messenger','morning_group','afternoon_return','treatment_items'];
