...
function dailyGuideMessages(cl,raw,people=[],history={},sourceDate=isoDay()){
 ...
 const evalTarget=evalsNeeded!=null?Math.min(15,Math.max(1,evalsNeeded)):15;
 const evalTargetText=evalsNeeded!=null?String(evalTarget):'até 15';
 const mapNames=new Set((x.collaborators||[]).map(v=>v.name));
 const assignment=team.map((p,i)=>({name:p.name.trim(),index:i,hasMap:mapNames.has(p.name.trim().toUpperCase())}));
 const newContacts=x.amigoPoss!=null&&x.amigoInd!=null?Math.max(0,Math.ceil(x.amigoPoss*.1)-x.amigoInd):null;
 const capture=x.amigoPct!=null?'A indicação está em '+Math.round(x.amigoPct)+'%'+(x.amigoPoss!=null?' ('+(x.amigoInd??'—')+' de '+x.amigoPoss+' possibilidades)':'')+'. ':'';
 ...
 const clinicPraise=clinicWins.length?'Ponto positivo: '+clinicWins.slice(0,2).join(' e ')+'. ':'';
 const healthyTicket=x.ticketHealthy||3488.25;
 const agendaToday=x.todayEvaluations!=null?'Hoje temos '+x.todayEvaluations+' avaliações na agenda. ':'O Mapa não mostra com segurança quantas avaliações já estão na agenda de hoje. ';
 const simpleYesterday=deltaCG!=null?'Ontem fechamos com '+deltaCG+' '+(deltaCG===1?'efetivação':'efetivações')+' em Clínico Geral'+(deltaOrtho!=null?' e '+deltaOrtho+' em Ortodontia':'')+'. ':'Antes de enviar, confirme o fechamento de ontem em Clínico Geral e Ortodontia. ';
 const firstName=n=>{const f=String(n||'').trim().split(/\s+/)[0]||'pessoal';return f.slice(0,1).toLocaleUpperCase('pt-BR')+f.slice(1).toLocaleLowerCase('pt-BR')};
 const pctSentence=(value,label,healthy)=>value!=null?label+' está em '+Math.round(value)+'%'+(healthy!=null?', e nosso saudável é '+healthy+'%':'')+'. ':'';
 const individual=assignment.map(a=>{
   const g=firstName(a.name),role=a.index%3;
   if(role===0){
     const need=chairDaily!=null?'Para acompanhar a meta da clínica, precisamos de '+chairDaily+' efetivações de CG por dia útil. ':'';
     const accept=pctSentence(x.accept,'A aceitação de CG',80);
     const current=x.todayEvaluations!=null?'Temos '+x.todayEvaluations+' avaliações marcadas para hoje. ':'';
     return {recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+g+', tudo bem? '+simpleYesterday+current+need+accept+'Seu foco hoje é encher a agenda de avaliações: trabalhe a conversão e deixe '+evalTargetText+' novas avaliações agendadas entre hoje e amanhã, sem passar de 15. O mais importante é não encerrar o dia com horários de avaliação vazios. '+clinicPraise+'No meio do dia me diga quantas avaliações você já conseguiu agendar, combinado?'};
   }
   if(role===1){
     const absent=x.missedYesterday!=null?'Ontem tivemos '+x.missedYesterday+' faltoso'+(x.missedYesterday===1?'':'s')+'. ':'';
     const noAgenda=x.noAgendaCG!=null?Math.round(x.noAgendaCG)+'% dos pacientes de Clínico Geral estão sem o próximo agendamento. ':'';
     const rebook=pctSentence(x.rebooking,'O reagendamento',90);
     return {recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+g+'! Tudo bem? '+simpleYesterday+absent+noAgenda+rebook+'Hoje quero você cuidando dos faltosos, olhando a agenda dos profissionais e chamando quem ainda está sem o primeiro/próximo agendamento. A ideia é aproveitar os horários disponíveis e não deixar paciente parado. '+clinicPraise+'Na parcial me passe quantos pacientes você recuperou e quantos ficaram agendados, combinado?'};
   }
   const exec=pctSentence(x.collectionExecution,'A execução da cobrança',80);
   const charge=x.charges!=null?'O relatório mostra '+x.charges+' cobranças realizadas'+(x.charged!=null?' e '+x.charged+' registros de pacientes cobrados':'')+'. ':'';
   const ortho=orthoGap!=null?'Na Ortodontia ainda faltam '+orthoGap+' pastas pagas para a meta do mês. ':'';
   return {recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+g+', tudo bem? '+simpleYesterday+exec+charge+ortho+'Seu foco hoje é cobrança. Puxe as pastas de Ortodontia em atraso dos últimos 3 meses, faça os contatos e acompanhe até ter uma resposta clara do paciente. Separe o que foi só tentativa, o que virou acordo e o que realmente foi pago. '+clinicPraise+'Na parcial me passe esses três números e me diga onde está a maior dificuldade, combinado?'};
 });
 for(const p of collectors){
   const name=p.name.trim(),dual=individual.find(v=>v.recipient.toLocaleLowerCase('pt-BR')===name.toLocaleLowerCase('pt-BR'));if(dual)continue;
   const exec=pctSentence(x.collectionExecution,'A execução da cobrança',80),charge=x.charges!=null?'O relatório mostra '+x.charges+' cobranças realizadas'+(x.charged!=null?' e '+x.charged+' pacientes cobrados':'')+'. ':'',ortho=orthoGap!=null?'Ainda faltam '+orthoGap+' pastas pagas de Ortodontia para a meta do mês. ':'';
   individual.push({recipient:name,title:'Mensageiro · cobrança · '+name,message:'Bom dia, '+firstName(name)+', tudo bem? '+exec+charge+ortho+'Hoje revise as pastas de Ortodontia atrasadas dos últimos 3 meses. Quero uma cobrança simples e objetiva, acompanhando cada caso até resposta, acordo ou pagamento. Na parcial me traga tentativas, acordos e pagamentos confirmados, separados. Combinado?'});
 }
 const group='Bom dia, equipe e franqueados de '+cl.city+'. Pelo Mapa até '+brDate(x.periodEnd)+', '+chairLine+moneyLine+orthoLine+focus+'Hoje dividimos o trabalho em três frentes: avaliações para hoje e amanhã (máximo de 15 novas), faltosos/agenda dos profissionais/pacientes sem agendamento e cobrança das pastas de Ortodontia atrasadas dos últimos 3 meses. '+clinicPraise+'Na parcial quero números simples: avaliações agendadas, pacientes recuperados, CG e Orto efetivados, acordos e pagamentos. Franqueados, onde a equipe precisa de apoio hoje?' ;
