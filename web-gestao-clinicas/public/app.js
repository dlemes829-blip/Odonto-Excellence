...
function dailyGuideMessages(cl,raw,people=[]){
 ...
 if(!rec.length){individual.splice(0,individual.length,{recipient:'Colaboradora 1 · editar nome',title:'Meta individual · Colaboradora 1',message:'Bom diaaa! 😄 Hoje preciso da sua ajuda para atacar avaliações, confirmações, reativação e follow-up. Sua meta individual é buscar '+Math.max(1,daily)+' efetivação(ões) no clínico e ajudar a transformar a agenda em paciente na cadeira. Na Orto, acompanhe avaliações e fechamento de pasta paga. Depois você só troca meu nome pelo nome correto da colaboradora, combinado? 🙌'});}
...
async function renderPeriod1(c){
 ...
 $('#p1Clinic').onchange=e=>{state.selectedClinic=Number(e.target.value);renderPeriod1(c)};$('#generateDaily').onclick=async()=>{await saveDailyGuides(id);renderPeriod1(c)};
 const a=await guideCards(id,'morning_messenger'),b=await guideCards(id,'morning_group');$('#morningGuides').innerHTML='<div class="guide-strip"><div><strong>Mensageiro individual</strong><span>Meta própria, vínculo e plano do dia.</span></div><div><strong>Grupo</strong><span>Leitura estratégica + solução + acompanhamento.</span></div></div><h3>Mensageiros</h3>'+a+'<h3>Grupo</h3>'+b;await bindGuideCards();
}
...
async function renderReturns(c){
 ...
 $('#rClinic').onchange=e=>{state.selectedClinic=Number(e.target.value);renderReturns(c)};$('#rGenerate').onclick=async()=>{await saveDailyGuides(id);renderReturns(c)};await bindGuideCards();
}
