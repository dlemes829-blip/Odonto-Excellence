...
function dailyGuideMessages(cl,raw,people=[],history={},sourceDate=isoDay()){
 ...
 const yesterday=deltaCG!=null?'Entre '+brDate(prev.periodEnd)+' e '+brDate(x.periodEnd)+', foram '+deltaCG+' efetivações de Clínico Geral'+(deltaOrtho!=null?' e '+deltaOrtho+' '+(deltaOrtho===1?'avaliação':'avaliações')+' de Orto efetivada'+(deltaOrtho===1?'':'s'):'')+'. O que dificultou os demais fechamentos? ':'';
