...
function dailyGuideMessages(cl,raw,people=[],history={},sourceDate=isoDay()){
 ...
 const yesterday=deltaCG!=null?'Entre '+brDate(prev.periodEnd)+' e '+brDate(x.periodEnd)+', foram '+deltaCG+' efetivações de Clínico Geral'+(deltaOrtho!=null?' e '+deltaOrtho+' avaliações de Orto efetivadas':'')+'. O que dificultou os demais fechamentos? ':'';
 ...
 const indicationTask='Peça indicações em todos os atendimentos, conforme a campanha vigente e com autorização do paciente. '+(x.amigoPoss!=null?'O Mapa aponta '+x.amigoPoss+' possibilidades de Amigo do Peito'+(x.amigoInd!=null?' e '+x.amigoInd+' indicações':'')+'. ':'')+'Quero o total de contatos novos e de avaliações agendadas.';
 ...
 const individual=assignment.map(a=>{const first=a.name.split(/\s+/)[0],greeting=first.slice(0,1).toLocaleUpperCase('pt-BR')+first.slice(1).toLocaleLowerCase('pt-BR');const target='Sua meta individual de hoje é '+(a.cg>0?a.cg+' '+(a.cg===1?'efetivação':'efetivações')+' de Clínico Geral, que '+(a.cg===1?'representa':'representam')+' '+centralMoney(a.cg*healthyTicket)+' ('+a.cg+' × ticket médio saudável de '+centralMoney(healthyTicket)+')':a.cg===0?'apoiar a agenda e os fechamentos de CG':'efetivações de Clínico Geral a definir')+(a.ortho>0?' e '+a.ortho+' '+(a.ortho===1?'pasta paga':'pastas pagas')+' de Orto':'')+'. ';return {recipient:a.name,title:'Mensageiro · '+a.name,message:'Bom dia, '+greeting+', tudo bem? '+yesterday+'No Mapa até '+brDate(x.periodEnd)+', houve '+(x.evals??'—')+' avaliações e '+(x.eff??'—')+' efetivações de CG; a aceitação está em '+(x.accept!=null?Math.round(x.accept)+'%':'apuração pendente')+' diante dos 80% saudáveis. '+daily+target+capture+a.task+' '+praise+'Como está sua agenda real de hoje? O que está dificultando e quantas avaliações você consegue agendar para hoje e amanhã? Me traga os números na parcial, combinado?'}});
...
async function guideCards(id,section){
 ...
 latest.reverse();
 return latest.map(r=>'<div class="card guide-output" data-paste-guide="'+r.id+'" tabindex="0"><div class="section-head"><div><strong>'+esc(r.title)+'</strong><div class="clinic-meta">'+esc(r.recipient)+'</div></div><div class="actions"><button class="ghost" data-save-guide="'+r.id+'">Salvar edição</button><button class="ghost" data-copy-guide="'+r.id+'">Copiar mensagem</button>'+(r.image_data?'<button class="btn primary" data-copy-guide-pack="'+r.id+'">Copiar print + mensagem</button>':'')+'</div></div><textarea class="message-box" rows="8" data-guide-text="'+r.id+'">'+esc(r.message)+'</textarea>'+(r.section==='treatment_items'?'<div class="hint">Clique neste cartão e cole o print com Ctrl+V. A imagem fica vinculada somente a esta mensagem.</div>':'')+'<label class="btn secondary">Adicionar imagem<input class="hidden guide-image" data-guide-image="'+r.id+'" type="file" accept="image/*"></label>'+(r.image_data?'<img class="guide-preview" src="'+r.image_data+'">':'')+'</div>').join('')}
