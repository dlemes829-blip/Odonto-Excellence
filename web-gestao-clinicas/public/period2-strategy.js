...
function renderAnalysis(){
 ...
 qa('[data-p2copy]').forEach(b=>b.onclick=()=>{const item=last.items.find(x=>x.reportId===b.dataset.p2copy);if(item)copy(item.message||insight(item,cl)?.message||'')});
...
function render(){
 ...
 <div class="p2-drop"><label><strong>Escolher 1 arquivo</strong><span>Imagem, print, PDF, TXT ou CSV</span><input id="p2Files" type="file" accept="image/*,.pdf,.txt,.csv"></label><div id="p2Paste" tabindex="0"><strong>Colar print/tela</strong><span>Copie o relatório e use Ctrl+V. A leitura começa na hora.</span></div><div id="p2ScreenPaste" tabindex="0"><strong>Colar conteúdo da tela</strong><span>Prioriza texto/HTML quando o relatório permitir copiar os dados.</span><button id="p2ClipboardRead" class="ghost" type="button">Ler área de transferência</button></div></div>
